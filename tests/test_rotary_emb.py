...
def test_create_rotation_block():
    ...
    block = create_rotation_block(angle)
...
def test_create_rotation_matrix():
    ...
    rotation_matrix = create_rotation_matrix(embeddings)
    ...
    pos = 0
    expected_block = create_rotation_block(np.power(10000, -2 * pos / embedding_dim))
    ...
    block = create_rotation_block(np.power(10000, -2 * (embedding_dim // 2 - 1) / embedding_dim))
